...
# 파일 읽기
df = pd.read_csv(r"격자별_변수_통합.csv")
station = pd.read_csv(r"MCLP에 넣는 후보역들_186개.csv")
gu = gpd.read_file(r"서울_자치구_경계_2017.geojson")
...
print("\n가중치 비교:")
print(f"{'시나리오':<12} {'생활이동인구':>12} {'스트라바':>10} {'거주인구':>10} {'녹시율':>10} {'공원겹침':>10}")
for name, w in scenarios.items():
    print(f"{name:<12} {w[0]:>12.4f} {w[1]:>10.4f} {w[2]:>10.4f} {w[3]:>10.4f} {w[4]:>10.4f}")
...
for name, weights in scenarios.items():
    print(f"\n[{name}] MCLP 최적화 중...")
    a = (df['생활이동인구_norm'] * weights[0] +
         df['스트라바이용자수_norm'] * weights[1] +
         df['거주인구_norm'] * weights[2] +
         df['녹시율_norm'] * weights[3] +
